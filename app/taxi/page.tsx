...
const DATES = [
  { v: "all", label: "Full week" },
  { v: "2008-02-02", label: "Sat Feb 02" },
  { v: "2008-02-03", label: "Sun Feb 03" },
  { v: "2008-02-04", label: "Mon Feb 04" },
  { v: "2008-02-05", label: "Tue Feb 05" },
  { v: "2008-02-06", label: "Wed Feb 06" },
  { v: "2008-02-07", label: "Thu Feb 07" },
  { v: "2008-02-08", label: "Fri Feb 08" },
];
...
export default function TaxiPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Taxi trajectory</h1>
        <p className="text-muted text-sm">
          Pick any of the {taxis.length} cleaned taxis from the dataset and filter by day and hour
          range. Each color represents a different trip (segments separated by more than 5 minutes
          of inactivity).
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-[320px_1fr] gap-4">
        <aside className="card space-y-3 max-h-[620px] overflow-hidden flex flex-col">
          <div className="card-title">Taxi selector</div>
          <input
            className="input"
          ...
          <div className="overflow-y-auto flex-1 border border-border rounded-md bg-bg/40">
            ...
            {filtered.length === 0 && <div className="text-xs text-muted p-3">No results.</div>}
            {filtered.length > 200 && (
              <div className="text-xs text-muted p-3">Showing 200 of {filtered.length}.</div>
            )}
          </div>
          <div>
            <div className="card-title !mb-1">Most active taxis</div>
          ...
          <div className="card">
            <div className="grid md:grid-cols-3 gap-4 items-end">
              ...
              <div className="md:col-span-2">
                <div className="flex items-baseline justify-between">
                  <label className="text-xs text-muted">Hour range</label>
            ...
            {selected && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 text-sm">
                ...
                <Meta label="Total points" value={selected.points.toLocaleString()} />
                <Meta label="First record" value={selected.first} />
                <Meta label="Last record" value={selected.last} />
              </div>
            )}
          </div>
